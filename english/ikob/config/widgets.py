...
def _browseFile(var):
  ...
  if len(var.get()) > 0:
    initpath = var.get()
  selectedfile = filedialog.askopenfilename(initialdir=initpath, title='Choose a file.')
  if selectedfile:
...
def _browseDir(var):
  ...
  if len(var.get()) > 0:
    initpath = var.get()
  selectedpath = filedialog.askdirectory(initialdir=initpath, title='Choose a director')
  if selectedpath:
