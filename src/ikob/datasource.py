...
from ikob.zone_id_store import ZoneIdStoreSingleton
...
class SkimsSource:
    ...
    def __init__(self, config, skims_dir: pathlib.Path | str):
        # TODO FIX THE IMPORTS HERE, THIS CAN ALL JUST BE CONFIG, AND THE READ CALLS DO NO NEED THE CONFIG ARGUMENT
        if skims_dir == "":
    ...
    def read(
        ...
        if os.path.exists(path):
            return utils.read_csv(
                ...
                has_id_column=has_id_column,
                zone_id_store=ZoneIdStoreSingleton.get_instance(self.config),
            )
        if default is None:
    ...
    def read_parking_times(self, config):
        ...
        if parking_time_path.exists():
            ...
            return utils.read_csv(parking_time_path, type_caster=utils.INT_DTYPE, zone_id_store=zone_id_store)
        ...
        urbanization_grade = utils.read_csv(urbanization_path, type_caster=utils.INT_DTYPE, zone_id_store=zone_id_store)
...
class DataSource:
    ...
    def read_csv(self, key: DataKey) -> NDArray:
        ...
        zone_id_store = ZoneIdStoreSingleton.get_instance(self.config)
        matrix, ids = utils.read_csv(path, zone_id_store=zone_id_store)
        matrix = zone_id_store.align_rows_to_skim_zone_ids(ids, matrix)
        return matrix
    ...
    def write_csv(self, data, key: DataKey):
        ...
        zone_id_store = ZoneIdStoreSingleton.get_instance(self.config)
        header = utils.header_from_zone_ids(zone_id_store.zone_ids)
        index = utils.CsvIdColumn.from_zone_ids(zone_id_store.zone_ids)
