...
def potential_companies(config, single_weights: DataSource, combined_weights: DataSource) -> DataSource:
    ...
    Corresponds to https://docs.crow.nl/ikob/algorithm/D5-potential-companies
    """
    ...
    if "werk" in motives:
        distribution_matrix = segs_source.read(
            "Verdeling_over_groepen_Beroepsbevolking", scenario=scenarios, type_caster=float
        )
    elif "winkelnietdagelijksonderwijs" in motives:
        distribution_matrix = segs_source.read(
            "Verdeling_over_groepen_Leerlingen", scenario=scenarios, type_caster=float
        )
    ...
    for car_possession_group in car_possession_groups:
        for motive in motives:
            ...
            if car_possession_group == "alle groepen":
                distribution_matrix = segs_source.read(
                    f"Verdeling_over_groepen_{target_group}", scenario=scenarios, type_caster=float
                )
            else:
                distribution_matrix = segs_source.read(
            ...
            for part_of_day in part_of_days:
                ...
                for modality in modalities:
                    ...
                    # https://docs.crow.nl/ikob/algorithm/D5-potential-companies defines $B_{irv}$ as a jobs-weighted aggregation over destination zones in a region.
