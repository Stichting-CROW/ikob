...
if Additionele_kosten:
    ...
    Additionele_kostenmatrix = utils.read_csv_int(
        Additionele_kostenfile, aantal_lege_regels=0)
...
for ds in dagsoort:
    ...
    Autotijdmatrix = utils.read_csv_float(
        Autotijdfilenaam, aantal_lege_regels=0)
    Autoafstandfilenaam = os.path.join(Invoerdirectory, f'Auto_Afstand')
    Autoafstandmatrix = utils.read_csv_float(
        Autoafstandfilenaam, aantal_lege_regels=0)
    Fietstijdfilenaam = os.path.join(Invoerdirectory, f'Fiets_Tijd')
    Fietstijdmatrix = utils.read_csv_float(
        Fietstijdfilenaam, aantal_lege_regels=0)
    ...
    OVafstandmatrix = utils.read_csv_float(
        OVafstandfilenaam, aantal_lege_regels=0)
    if Parkeerkosten:
        Parkeerkostenfile = Parkeerkostenfile.replace('.csv', '')
        Parkeerkostenlijst = utils.read_csv_int(
            Parkeerkostenfile, aantal_lege_regels=0)
    else:
    ...
    if Ketens:
        Pplusfietstijdfilenaam = os.path.join(
            Invoerdirectory, f'Pplusfiets_{Hubnaam}_Tijd')
        Pplusfietstijdmatrix = utils.read_csv_float(
            Pplusfietstijdfilenaam, aantal_lege_regels=0)
        Pplusfietsafstandfilenaam = os.path.join(
            Invoerdirectory, f'Pplusfiets_{Hubnaam}_Afstand_Auto')
        Pplusfietsafstandmatrix = utils.read_csv_float(
            Pplusfietsafstandfilenaam, aantal_lege_regels=0)
        PplusRbestemmingstijdfilenaam = os.path.join(
            Invoerdirectory, f'PplusR_{Hubnaam}_bestemmings_Tijd')
        PplusRbestemmingstijdmatrix = utils.read_csv_float(
            PplusRbestemmingstijdfilenaam, aantal_lege_regels=0)
        PplusRherkomsttijdfilenaam = os.path.join(
            Invoerdirectory, f'PplusR_{Hubnaam}_herkomst_Tijd')
        PplusRherkomsttijdmatrix = utils.read_csv_float(
            PplusRherkomsttijdfilenaam, aantal_lege_regels=0)
        PplusRbestemmingsOVafstandfilenaam = os.path.join(
            Invoerdirectory, f'PplusR_{Hubnaam}_bestemmings_Afstand_OV')
        ...
        PplusRherkomstOVafstandfilenaam = os.path.join(
            Invoerdirectory, f'PplusR_{Hubnaam}_herkomst_Afstand_OV')
        PplusRherkomstOVafstandmatrix = utils.read_csv_float(
            PplusRherkomstOVafstandfilenaam, aantal_lege_regels=0)
        PplusRherkomstautoafstandfilenaam = os.path.join(
            Invoerdirectory, f'PplusR_{Hubnaam}_herkomst_Afstand_Auto')
        PplusRherkomstautoafstandmatrix = utils.read_csv_float(
            PplusRherkomstautoafstandfilenaam, aantal_lege_regels=0)
        ...
        KostenbestemmingsPplusROV = [
            [
                KostenOV(
                    ...
                    OVkmtarief,
                    starttarief,
                ) for j in range(afmeting)] for i in range(afmeting)]
            ...
            for j in range(0, aantal_zones):
                ...
                if Additionele_kosten:
                    GGRskim[i].append(int(totaleTijd +
                                          ...
                                          (Autoafstandmatrix[i][j] *
                                           (varautotarief +
                                            kmheffing) +
                                              Additionele_kostenmatrix[i][j] /
                                              100) +
                                          Parkeerkostenlijst[j] /
                                          100))
                else:
                    GGRskim[i].append(int(totaleTijd +
                                          Vermenigvuldigingsfactor *
        ...
        for i in range(0, aantal_zones):
            ...
            for j in range(0, aantal_zones):
                if float(OVtijdmatrix[i][j]) > 0.5:
                    Resultaat = float(
                        OVtijdmatrix[i][j]) + Vermenigvuldigingsfactor * float(KostenmatrixOV[i][j])
            ...
            for i in range(0, aantal_zones):
                ...
                for j in range(0, aantal_zones):
                    if Autotijdmatrix[i][j] < 7:
                        ...
                        GGRskim[i].append(
                            int(totaleTijd + Vermenigvuldigingsfactor * totaleKosten))
                ...
                for j in range(0, aantal_zones):
                    ...
                    if Additionele_kosten:
                        ...
                        GGRskim[i].append(int(totaleTijd +
                                              ...
                                              Parkeerkostenlijst[j] /
                                              100))
            Uitvoerfilenaam = os.path.join(
                Uitvoerdirectory, f'GratisAuto_{ink}')
            utils.write_csv(GGRskim, Uitvoerfilenaam)
        ...
        if Ketens:
            for ink in inkomens:
                ...
                for i in range(0, aantal_zones):
                    ...
                    for j in range(0, aantal_zones):
                        if Additionele_kosten:
                            GGRskim[i].append(int(Pplusfietstijdmatrix[i][j] +
                                                  Vermenigvuldigingsfactor *
                ...
                Uitvoerfilenaam = os.path.join(
                    Uitvoerdirectory, f'Pplusfiets_{Hubnaam}_{ink}')
                utils.write_csv(GGRskim, Uitvoerfilenaam)
                ...
                Uitvoerfilenaam = os.path.join(
                    Uitvoerdirectory, f'PplusRbestemmings_{Hubnaam}_{ink}')
                utils.write_csv(GGRskim, Uitvoerfilenaam)
                ...
                Uitvoerfilenaam = os.path.join(
                    Uitvoerdirectory, f'PplusRherkomst_{Hubnaam}_{ink}')
                utils.write_csv(GGRskim, Uitvoerfilenaam)
