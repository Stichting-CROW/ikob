...
income_distribution_path = os.path.join(
    segs_directory, 'Inkomensverdeling_per_zone')
citizens_count_path = os.path.join(segs_directory, f'Beroepsbevolking{year}')
incomde_distribution = utils.read_csv_int(
    income_distribution_path, aantal_lege_regels=1)
citizens = utils.read_csv_int(citizens_count_path)
citizens_per_zone = []
for i in range(len(citizens)):
    citizens_per_zone.append([])
    for j in range(len(incomde_distribution[0])):
        citizens_per_zone[i].append(
            int(citizens[i] * incomde_distribution[i][j] / 100))
citizens_class_zone_file = os.path.join(
    segs_directory, f'Inwoners_per_klasse{year}')
