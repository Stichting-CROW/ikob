...
skims.label = (
    "Voer de directory waar de pure reistijdskims en afstandskims in staan")
skims.directory = filedialog.askdirectory(
    initialdir=os.getcwd(), title="Selecteer de directory skimsdirectory",)
skims.destroy()
...
Autoafstandmatrix = Routines.csvlezen(
    Autoafstandfilenaam, aantal_lege_regels=0)
...
while Hub > -1:
    Hub = int(input(
        'Geef de zone(s) met hub(s) één voor één aan. Als je de laatste hebt gehad, typ dan -1'))
    if Hub > 0:
        # De nummering van een vector of matrix begint in Python bij kolom of rij 0
        Hubset.append(Hub-1)
...
OverstaptijdOV = int(
    input('Hoeveel overstaptijd is er op de hub tussen auto en OV?'))
Overstaptijdfiets = int(
    input('Hoeveel overstaptijd is er op de hub tussen auto en fiets?'))
Pplusfietshubplek = []
PplusRhubplek = []
        ...
        for j in range(len(Autotijdmatrix)):
            if OVtijdmatrix[i][Hubset[h]] <= Autotijdmatrix[Hubset[h]][j]:
                PplusRbestemmingstijdmatrix[h][i].append(round(OVtijdmatrix[i][Hubset[h]] + Autotijdmatrix[Hubset[h]][j]
                                                               + OverstaptijdOV))
                PplusRbestemmingsafstandautomatrix[h][i].append(
                    round(Autoafstandmatrix[Hubset[h]][j]))
                PplusRbestemmingsafstandOVmatrix[h][i].append(
                    round(OVafstandmatrix[i][Hubset[h]]))
                PplusRherkomsttijdmatrix[h][i].append(round(Autotijdmatrix[i][Hubset[h]] + OVtijdmatrix[Hubset[h]][j]
                                                            + OverstaptijdOV))
                PplusRherkomstafstandautomatrix[h][i].append(
                    round(Autoafstandmatrix[i][Hubset[h]]))
                PplusRherkomstafstandOVmatrix[h][i].append(
                    round(OVafstandmatrix[Hubset[h]][j]))
            else:
...
PplusRbestemmingstijdfilenaam = os.path.join(
    Skimsdirectory, f'PplusR_{Hubsetnaam}_bestemmings_Tijd')
Routines.csvwegschrijven(PplusRbestemmingstijdtotaal,
                         PplusRbestemmingstijdfilenaam)
PplusRbestemmingsafstandautofilenaam = os.path.join(
    Skimsdirectory, f'PplusR_{Hubsetnaam}_bestemmings_Afstand_Auto')
Routines.csvwegschrijven(PplusRbestemmingsafstandautototaal,
                         PplusRbestemmingsafstandautofilenaam)
PplusRbestemmingsafstandOVfilenaam = os.path.join(
    Skimsdirectory, f'PplusR_{Hubsetnaam}_bestemmings_Afstand_OV')
Routines.csvwegschrijven(PplusRbestemmingsafstandOVtotaal,
                         PplusRbestemmingsafstandOVfilenaam)
PplusRherkomsttijdfilenaam = os.path.join(
    Skimsdirectory, f'PplusR_{Hubsetnaam}_herkomst_Tijd')
Routines.csvwegschrijven(PplusRherkomsttijdtotaal, PplusRherkomsttijdfilenaam)
PplusRherkomstafstandautofilenaam = os.path.join(
    Skimsdirectory, f'PplusR_{Hubsetnaam}_herkomst_Afstand_Auto')
Routines.csvwegschrijven(PplusRherkomstafstandautototaal,
                         PplusRherkomstafstandautofilenaam)
PplusRherkomstafstandOVfilenaam = os.path.join(
    Skimsdirectory, f'PplusR_{Hubsetnaam}_herkomst_Afstand_OV')
Routines.csvwegschrijven(PplusRherkomstafstandOVtotaal,
                         PplusRherkomstafstandOVfilenaam)
Pplusfietstijdfilenaam = os.path.join(
    Skimsdirectory, f'Pplusfiets_{Hubsetnaam}_Tijd')
Routines.csvwegschrijven(PplusFietstijdtotaal, Pplusfietstijdfilenaam)
Pplusfietsautoafstandfilenaam = os.path.join(
    Skimsdirectory, f'Pplusfiets_{Hubsetnaam}_Afstand_Auto')
Routines.csvwegschrijven(PplusFietsautoafstandtotaal,
                         Pplusfietsautoafstandfilenaam)
Pplusfietshubplekfilenaam = os.path.join(
    Skimsdirectory, f'Pplusfiets_{Hubsetnaam}_bestehubs')
Routines.csvwegschrijven(Pplusfietshubplek, Pplusfietshubplekfilenaam)
PplusRhubplekfilenaam = os.path.join(
    Skimsdirectory, f'PplusR_{Hubsetnaam}_bestehubs')
