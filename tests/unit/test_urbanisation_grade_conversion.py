...
def test_stedelijkheid_converter():
    ...
    reference = read_csv(segs_dir / "Parkeerzoektijd.csv", type_caster=INT_DTYPE)
    stedelijkheid = read_csv(segs_dir / "Stedelijkheidsgraad.csv", type_caster=INT_DTYPE)
...
def test_generate_parkeerzoektijden():
    ...
    reference = read_csv(project_dir / "SEGS" / "Parkeerzoektijd.csv", type_caster=INT_DTYPE)
    config = get_config_from_args(project_file)

    skims = SkimsSource(config, config["project"]["paden"]["skims_directory"])

    # Read test file on disk given in configuration file.
    assert np.all(reference == skims.read_parking_times(config))
