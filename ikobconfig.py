...
class ConfigApp(tk.Tk):
  ...
  def cmdOpslaanProject(self):
    ...
    try:
      ...
      messagebox.showerror(title='Fout', message=f'Het bestand kan niet worden opgeslagen.')
    else:
...
def main():
  ...
  if not validate.validateTemplate(StandaardConfiguratieDefinitie()):
    messagebox.showerror(title='Fout', message='De standaard configuratiedefinitie is niet geldig: Kijk in ConfiguratieDefinitie.py')
    exit(1)
  App = ConfigApp()
  App.mainloop()
